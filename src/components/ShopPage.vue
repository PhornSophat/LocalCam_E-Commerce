        ...
        <div class="grid grid-cols-2 gap-8 md:grid-cols-3 lg:grid-cols-5">
          ...
          >
            <div class="relative overflow-hidden transition-all duration-500 shadow-lg aspect-square bg-linear-to-br from-stone-100 to-stone-200 rounded-2xl hover:shadow-2xl">
              <img :src="product.image_url" :alt="product.name" class="object-cover w-full h-full transition-transform duration-700 ease-out group-hover:scale-110" />
              
              <div v-if="product.stock <= 0" class="absolute inset-0 z-20 flex items-center justify-center bg-black/40 backdrop-blur-[2px]">
                <span class="px-4 py-2 text-xs font-bold text-white uppercase border-2 border-white rounded-lg">Sold Out</span>
              </div>
              ...
              <div class="absolute flex flex-col gap-2 transition-all duration-300 translate-x-4 opacity-0 top-4 right-4 group-hover:opacity-100 group-hover:translate-x-0">
                <button class="flex items-center justify-center w-10 h-10 transition-colors bg-white rounded-full shadow-lg hover:bg-red-50 text-stone-700 hover:text-red-500">
            ...
            <div class="px-1 mt-4">
              <div class="flex items-start justify-between mb-1">
                <h3 class="text-sm font-semibold transition-colors text-stone-800 group-hover:text-amber-700">{{ product.name }}</h3>
                <p class="text-base font-bold text-stone-900">${{ product.price }}</p>
              </div>
              
              <p v-if="product.stock > 0 && product.stock < 5" class="text-[10px] font-bold text-red-500 animate-pulse uppercase">
                Only {{ product.stock }} left!
              </p>
              <p v-else-if="product.stock >= 5" class="text-[10px] text-stone-400 font-medium">
                In Stock: {{ product.stock }}
              </p>
              <p v-else class="text-[10px] text-red-700 font-bold uppercase">
                Restocking Soon
              </p>
            </div>
          </div>
